...
const analyzeSearchHistory = require('./analyzeSearchHistory'); // Adjust the path as necessary
...
app.post('/search', async (req, res) => {
    ...
    try {
      await client.query('INSERT INTO search_history (user_id, query, timestamp) VALUES ($1, $2, NOW())', [userId, query]);
      const result = await client.query('SELECT * FROM products WHERE name ILIKE $1', [`%${query}%`]);
      
      // Call the recommendation analysis function
      await analyzeSearchHistory(userId, query);
  ...
  // Endpoint to fetch personalized recommendations
  app.get('/recommendations/:userId', async (req, res) => {
    ...
    try {
      const result = await pool.query(`
        SELECT p.*
        FROM products p
        JOIN recommendations r ON p.productid = r.product_id
        WHERE r.user_id = $1
        LIMIT 10
      `, [userId]);
      res.json(result.rows);
